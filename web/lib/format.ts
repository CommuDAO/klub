...
export const dateRange = (start?: bigint, end?: bigint, locale?: string) => {
  if (!start) return "";
  ...
  const from = s.toLocaleTimeString(locale, { hour: "2-digit", minute: "2-digit" });
  const to = e ? e.toLocaleTimeString(locale, { hour: "2-digit", minute: "2-digit" }) : "";
  return `${day} · ${from}${to ? ` – ${to}` : ""}`;
};

export const dayLabel = (start?: bigint, locale?: string) =>
  start
    ? new Date(Number(start) * 1000).toLocaleDateString(locale, { day: "numeric", month: "long", weekday: "long" })
    : "";

export const timeOf = (seconds?: bigint, locale?: string) =>
  seconds && seconds > 0n ? new Date(Number(seconds) * 1000).toLocaleTimeString(locale) : "—";
